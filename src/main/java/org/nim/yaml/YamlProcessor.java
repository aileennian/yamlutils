...
public abstract class YamlProcessor {
	 ...
	 * separated by {@code ---} lines, and each document is converted* to properties before the match is made. E.g.
	 * <pre class="code"> environment: dev url: https://dev.bar.com
	 ...
	 * name: My Cool App
	 * </pre>
	 * when mapped with* <pre class="code">* setDocumentMatchers(properties ->  ("prod".equals(properties.getProperty("environment")) ? MatchStatus.FOUND : MatchStatus.NOT_FOUND));
...
.	 */
	...
	/**
	 * Set locations of YAML {@link Resource resources}
 to be loaded.
 ...
 will be thrown when the corresponding YAML
	 * node is processed.
	 ...
	 * the {@link #setDocumentMatchers(DocumentMatcher...) matchers}
. If a document
	 * matches it is passed into the callback, along with its representation as Properties.
	 * Depending on the {@link #setResolutionMethod(ResolutionMethod)}
 not all of the
...
,
 ...
 will be thrown when the node is processed.
	 ...
	 * source. When called with the Map from a {@link MatchCallback}
 the result will
	 * contain the same values as the {@link MatchCallback}
 Properties.
...
.
 ...
 items, returning the most specific status.
	...
	/**
	 * {@link Constructor}
 that supports filtering of unsupported types.
	 * <p>If an unsupported type is encountered in a YAML document, an
	 * {@link IllegalStateException}
 will be thrown from {@link #getClassForName}
.
	 */


	private class FilteringConstructor extends Constructor {		FilteringConstructor(LoaderOptions loaderOptions) {
			super(loaderOptions);
